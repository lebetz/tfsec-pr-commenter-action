...
curl -s "$(curl -s -N https://api.github.com/repos/aquasecurity/tfsec/releases${TFSEC_VERSION} | grep -m 1 -o -E "https://.+?tfsec-linux-amd64" | head -n1)" -o tfsec-linux-amd64
curl -s "$(curl -s -N https://api.github.com/repos/aquasecurity/tfsec/releases${TFSEC_VERSION} | grep -m 1 -o -E "https://.+?tfsec_checksums.txt" | head -n1)" -o tfsec.checksums
...
curl -s "$(curl -s -N https://api.github.com/repos/aquasecurity/tfsec-pr-commenter-action/releases/${COMMENTER_VERSION} | grep -o -E "https://.+?commenter-linux-amd64")" -o commenter-linux-amd64
curl -s "$(curl -s -N https://api.github.com/repos/aquasecurity/tfsec-pr-commenter-action/releases/${COMMENTER_VERSION} | grep -o -E "https://.+?checksums.txt")" -o commenter.checksums
